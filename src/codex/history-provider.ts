 ...
 * still refuses — therefore needs the failure supplied rather than provoked, or the test
 * would assert the host's SQLite build instead of this decision (#4943).
 */
let stateDbPreflightOpenFailureForTests: ((path: string) => unknown) | undefined;
export function setStateDbPreflightOpenFailureForTests(hook: typeof stateDbPreflightOpenFailureForTests): void {
  stateDbPreflightOpenFailureForTests = hook;
}

/**
 * Read-only open flags that skip WAL shared memory entirely.
 *
 * `immutable=1` has to arrive as a `file:` URI, and a URI filename needs
 * SQLITE_OPEN_URI. Same idiom as the storage scanner, the log-guard inspector and the
 * coordinator doctor, for the same reason: never touch a foreign store's WAL protocol.
 */
...
function isStateDbCantOpenError(error: unknown): boolean {
 ...
 *
 * `{ readonly: true }` is tried FIRST and stays the primary path, because it is the only
 * mode that can see a live WAL: it joins the writer's shared memory, so a thread another
 * process just migrated to paginated history is visible here and the preflight refuses on
 * it. An immutable open reads the last checkpointed main database instead. Making that the
 * primary path would trade a refusal for a stale snapshot, and a refusal this preflight
 * fails to observe is a config transition that proceeds over history Codex owns — the
 * exact outcome the whole guard exists to prevent.
 *
 * A WAL store closed cleanly is the case that has no shared memory to join. SQLite cannot
 * create the `-shm` from a read-only connection, so the open fails SQLITE_CANTOPEN on a
 * store that is perfectly healthy, the catch-all folds that into
 * `history_injection_preflight_unavailable`, and `ocx sync` refuses on every attempt with
 * no way forward (#4943).
 *
 * So the fallback is admitted only in the state where the absent sidecars are what make an
 * immutable read exact rather than stale: no `-wal` and no `-shm` on disk means no writer
 * is attached and no committed content sits outside the main database, so the main file IS
 * the whole store and the two modes cannot disagree. Either sidecar present keeps the
 * original error and the refusal that follows from it — a `-wal` holds content this
 * connection would not read, and a `-shm` means a writer is attached, and neither is a
 * store this preflight may inspect from a snapshot.
 */
function openCodexStateForPreflight(resolvedPath: string): Database {
  try {
    const forced = stateDbPreflightOpenFailureForTests?.(resolvedPath);
